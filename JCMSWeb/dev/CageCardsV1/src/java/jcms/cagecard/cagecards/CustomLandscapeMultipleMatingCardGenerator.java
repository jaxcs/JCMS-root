 ...
 * @author mkamato
 */
public class CustomLandscapeMultipleMatingCardGenerator extends MatingCardTemplate{
    ...
    public byte[] buildCards(CageCardResultDTO CCRDTO, HttpServletResponse HSR, String ContainerID) {
        try {
            ...
            containerID = new Integer(MCRDTO.getStart()).toString();

            for (Result mr : MCRDTO.getMatingResults()) {
                if (MCRDTO.getGoodCards().contains(containerID)) {

                    theBuilder = new MatingQueueBuilder(mr.getRows(), CCRDTO.getDbSetupResults().getRows(), CCRDTO.getMyWorkgroups());
                    ...
                    table.setTotalWidth(360f);
                    table.setLockedWidth(true);
                    if (MCRDTO.getFormatType().equals(FormatType.alignCenter)) {
                        table.setHorizontalAlignment(Element.ALIGN_CENTER);
                    } else if (MCRDTO.getFormatType().equals(FormatType.alignLeft)) {
                        table.setHorizontalAlignment(Element.ALIGN_LEFT);
                    } else if (MCRDTO.getFormatType().equals(FormatType.alignRight)) {
                        table.setHorizontalAlignment(Element.ALIGN_RIGHT);
                    }
