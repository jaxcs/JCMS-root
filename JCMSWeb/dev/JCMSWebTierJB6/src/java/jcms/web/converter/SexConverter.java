...
/**
 * <b>File name:</b> SexConverter.java  <p>
 ...
 */
public class SexConverter extends WTBaseConverter implements Converter {
    ...
    public String getAsString(FacesContext context, UIComponent component, Object value) {
        ...
        try {
            if (value != null && (!value.equals(""))) {
                key = (((CvSexEntity) value).getKey()).toString();
            }
        } catch (Exception e) {
            FacesMessage message = this.getMessage(context, "Invalid Value");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
     ...
     *      given data type key.
     * Overview:  Converts the unique String representation of CvSexEntity
     *      to the actual object.
     * @param key CvSexEntity primary key
     * @return CvSexEntity object
     */
    private CvSexEntity findEntity(String key) {
        CvSexEntity dataTypeObject = null;

        try {
            for (CvSexEntity e : this.getListSupportDTO().getCvSex()) {
                if (key.equalsIgnoreCase(e.getSexKey().toString())) {
                    dataTypeObject = e;
                    break;
                }
            }
