 ...
 * @author mkamato
 */
public class CvMouseUseConverter extends WTBaseConverter implements Converter {
    
    MouseUseDAO dao = new MouseUseDAO();
    ...
    public String getAsString(FacesContext context, UIComponent component, Object value) {
        ...
        try {
            if (value != null && (!value.equals(""))) {
                key = ((cvMouseUseDTO) value).getMouseUseKey();
            }
        } catch (Exception e) {
            FacesMessage message = this.getMessage(context, "Invalid Value");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
     ...
     */
    private cvMouseUseDTO findEntity(String key) {
        cvMouseUseDTO dataTypeObject = null;

        ArrayList<cvMouseUseDTO> uses = dao.getCvMouseUses();
        try {
            for (cvMouseUseDTO e : uses) {
                if (key.equalsIgnoreCase(e.getMouseUseKey())) {
                    dataTypeObject = e;
                    break;
                }
            }
