...
/**
 * <b>File name:</b> PenConverter.java  <p>
 ...
 */
public class PenConverter extends WTBaseConverter implements Converter {
    ...
    public String getAsString(FacesContext context, UIComponent component, Object value) {
        ...
        try {
            if (value != null && (!value.equals(""))) {
                key = (((ContainerEntity) value).getKey()).toString();
            }
        } catch (Exception e) {
            FacesMessage message = this.getMessage(context, "Invalid Value");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
     ...
     *      given data type key.
     * Overview:  Converts the unique String representation of ContainerEntity
     *      to the actual object.
     * @param key ContainerEntity primary key
     * @return ContainerEntity object
     */
    private ContainerEntity findEntity(String key) {
        ContainerEntity dataTypeObject = null;

        try {
            for (ContainerEntity e : this.getListSupportDTO().getAllPens()) {
                if (key.equalsIgnoreCase(e.getContainerKey().toString())) {
                    dataTypeObject = e;
                    break;
                }
            }
