...
/**
 * <b>File name:</b> GenesConverter.java  <p>
 ...
 */
public class GenesConverter extends WTBaseConverter implements Converter {
    ...
    public String getAsString(FacesContext context, UIComponent component, Object value) {
        ...
        try {
            if (value != null && (!value.equals(""))) {
                key = (((GeneEntity) value).getKey()).toString();
            }
        } catch (Exception e) {
            FacesMessage message = this.getMessage(context, "Invalid Value");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
     ...
     */
    private GeneEntity findEntity(String key) {
        GeneEntity dataTypeObject = null;

        try {
            for (GeneEntity e : this.getListSupportDTO().getAllGenes()) {
                if (key.equalsIgnoreCase(e.getGeneKey().toString())) {
                    dataTypeObject = e;
                    break;
                }
            }
