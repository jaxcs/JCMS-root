...
/**
 * <b>File name:</b> MatingIDConverter.java  <p>
 ...
 */
public class MatingIDConverter extends WTBaseConverter implements Converter {
    ...
    public String getAsString(FacesContext context, UIComponent component,
        ...
        try {
            if (value != null && (!value.equals(""))) {
                key = (((MatingEntity) value).getKey()).toString();
            }
        } catch (Exception e) {
            FacesMessage message = this.getMessage(context, "Invalid Value");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
     ...
     *      given data type key.
     * Overview:  Converts the unique String representation of MatingEntity
     *      to the actual object.
     * @param key MatingEntity primary key
     * @return MatingEntity object
     */
    private MatingEntity findEntity(String key) {
        MatingEntity dataTypeObject = null;

        try {
            for (MatingEntity e : this.getListSupportDTO().getAllMatingIDs()) {
                if (key.equalsIgnoreCase(e.getMatingKey().toString())) {
                    dataTypeObject = e;
                    break;
                }
            }
