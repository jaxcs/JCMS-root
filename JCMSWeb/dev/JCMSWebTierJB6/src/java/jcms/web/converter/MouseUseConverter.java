...
/**
 * <b>File name:</b> MouseUseConverter.java  <p>
 ...
 */
public class MouseUseConverter extends WTBaseConverter implements Converter {
    ...
    public String getAsString(FacesContext context, UIComponent component, Object value) {
        ...
        try {
            if (value != null && (!value.equals(""))) {
                key = (((CvMouseUseEntity) value).getKey()).toString();
            }
        } catch (Exception e) {
            FacesMessage message = this.getMessage(context, "Invalid Value");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
     ...
     */
    private CvMouseUseEntity findEntity(String key) {
        CvMouseUseEntity dataTypeObject = null;

        try {
            for (CvMouseUseEntity e : this.getListSupportDTO().getAllCvMouseUse()) {
                if (key.equalsIgnoreCase(e.getMouseUsekey().toString())) {
                    dataTypeObject = e;
                    break;
                }
            }
