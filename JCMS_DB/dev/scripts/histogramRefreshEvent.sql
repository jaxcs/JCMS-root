...
DROP EVENT IF EXISTS `histoEvent`;
DELIMITER $$  

CREATE EVENT histoEvent
	ON SCHEDULE 
		EVERY 24 HOUR STARTS DATE_ADD(DATE(DATE_ADD(NOW(), INTERVAL 1 DAY)), INTERVAL '03:00:00' HOUR_SECOND)
	COMMENT 'Creates data for HistogramDataTable'
	DO
	BEGIN
		DECLARE idx INT Default 1 ;
		DECLARE theDay DATETIME DEFAULT NOW();

		DELETE FROM `HistogramDataTable` WHERE groupType != '';
